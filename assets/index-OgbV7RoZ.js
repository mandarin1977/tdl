 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Jt="WebChannelConnection";class PS extends SS{constructor(e){super(e),this.c_=[],this.forceLongPolling=e.forceLongPolling,this.autoDetectLongPolling=e.autoDetectLongPolling,this.useFetchStreams=e.useFetchStreams,this.longPollingOptions=e.longPollingOptions}Jo(e,n,s,i,r){const o=Hc();return new Promise(((l,c)=>{const u=new w_;u.setWithCredentials(!0),u.listenOnce(E_.COMPLETE,(()=>{try{switch(u.getLastErrorCode()){case ra.NO_ERROR:const p=u.getResponseJson();ce(Jt,`XHR for RPC '${e}' ${o} received:`,JSON.stringify(p)),l(p);break;case ra.TIMEOUT:ce(Jt,`RPC '${e}' ${o} timed out`),c(new fe(X.DEADLINE_EXCEEDED,"Request time out"));break;case ra.HTTP_ERROR:const m=u.getStatus();if(ce(Jt,`RPC '${e}' ${o} failed with status:`,m,"response text:",u.getResponseText()),m>0){let _=u.getResponseJson();Array.isArray(_)&&(_=_[0]);const T=_?.error;if(T&&T.status&&T.message){const A=(function(k){const F=k.toLowerCase().replace(/_/g,"-");return Object.values(X).indexOf(F)>=0?F:X.UNKNOWN})(T.status);c(new fe(A,T.message))}else c(new fe(X.UNKNOWN,"Server responded with status "+u.getStatus()))}else c(new fe(X.UNAVAILABLE,"Connection failed."));break;default:ve(9055,{l_:e,streamId:o,h_:u.getLastErrorCode(),P_:u.getLastError()})}}finally{ce(Jt,`RPC '${e}' ${o} completed.`)}}));const d=JSON.stringify(i);ce(Jt,`RPC '${e}' ${o} sending request:`,i),u.send(n,"POST",d,s,15)}))}T_(e,n,s){const i=Hc(),r=[this.Uo,"/","google.firestore.v1.Firestore","/",e,"/channel"],o=b_(),l=C_(),c={httpSessionIdParam:"gsessionid",initMessageHeaders:{},messageUrlParams:{database:`projects/${this.databaseId.projectId}/databases/${this.databaseId.database}`},sendRawJson:!0,supportsCrossDomainXhr:!0,internalChannelParams:{forwardChannelRequestTimeoutMs:6e5},forceLongPolling:this.forceLongPolling,detectBufferingProxy:this.autoDetectLongPolling},u=this.longPollingOptions.timeoutSeconds;u!==void 0&&(c.longPollingTimeout=Math.round(1e3*u)),this.useFetchStreams&&(c.useFetchStreams=!0),this.jo(c.initMessageHeaders,n,s),c.encodeInitMessageHeaders=!0;const d=r.join("");ce(Jt,`Creating RPC '${e}' stream ${i}: ${d}`,c);const p=o.createWebChannel(d,c);this.I_(p);let m=!1,_=!1;const T=new RS({Yo:y=>{_?ce(Jt,`Not sending because RPC '${e}' stream ${i} is closed:`,y):(m||(ce(Jt,`Opening RPC '${e}' stream ${i} transport.`),p.open(),m=!0),ce(Jt,`RPC '${e}' stream ${i} sending:`,y),p.send(y))},Zo:()=>p.close()}),A=(y,k,F)=>{y.listen(k,(U=>{try{F(U)}catch(j){setTimeout((()=>{throw j}),0)}}))};return A(p,Nr.EventType.OPEN,(()=>{_||(ce(Jt,`RPC '${e}' stream ${i} transport opened.`),T.o_())})),A(p,Nr.EventType.CLOSE,(()=>{_||(_=!0,ce(Jt,`RPC '${e}' stream ${i} transport closed`),T.a_(),this.E_(p))})),A(p,Nr.EventType.ERROR,(y=>{_||(_=!0,nr(Jt,`RPC '${e}' stream ${i} transport errored. Name:`,y.name,"Message:",y.message),T.a_(new fe(X.UNAVAILABLE,"The operation could not be completed")))})),A(p,Nr.EventType.MESSAGE,(y=>{if(!_){const k=y.data[0];Ke(!!k,16349);const F=k,U=F?.error||F[0]?.error;if(U){ce(Jt,`RPC '${e}' stream ${i} received error:`,U);const j=U.status;let se=(function(w){const C=Tt[w];if(C!==void 0)return rv(C)})(j),Ie=U.message;se===void 0&&(se=X.INTERNAL,Ie="Unknown error status: "+j+" with message "+U.message),_=!0,T.a_(new fe(se,Ie)),p.close()}else ce(Jt,`RPC '${e}' stream ${i} received:`,k),T.u_(k)}})),A(l,T_.STAT_EVENT,(y=>{y.stat===kc.PROXY?ce(Jt,`RPC '${e}' stream ${i} detected buffering proxy`):y.stat===kc.NOPROXY&&ce(Jt,`RPC '${e}' stream ${i} detected no buffering proxy`)})),setTimeout((()=>{T.__()}),0),T}terminate(){this.c_.forEach((e=>e.close())),this.c_=[]}I_(e){this.c_.push(e)}E_(e){this.c_=this.c_.filter((n=>n===e))}}function nc(){return typeof document<"u"?document:null}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
